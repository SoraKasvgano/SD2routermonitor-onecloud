...
const char *ssid = "wifi";         // 连接WiFi名（此处使用taichi-maker为示例）
                                    // 请将您需要连接的WiFi名填入引号中
const char *password = "pass"; // 连接WiFi密码（此处使用12345678为示例）
...
void getMemoryUsage()
{
    if (getNetDataInfo("mem.available", netChartData))
    {
        ...
        mem_usage = 100 * (1.0 - netChartData.max / 988.0);
    }
}
...
void getTemperature()
{
    if (getNetDataInfo("sensors.temp_iio_hwmon_hwmon_hwmon0_iio_hwmon", netChartData))
    {
